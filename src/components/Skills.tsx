import { 
  Code2, ShieldCheck, LayoutTemplate, Database, Cloud, Wrench 
} from 'lucide-react'
import {
  SiJavascript, SiTypescript, SiPython, SiPhp,
  SiFastapi, SiLaravel, SiDotnet, SiJsonwebtokens, SiBurpsuite, SiWireshark, SiKalilinux,
  SiReact, SiTailwindcss, SiLivewire, SiAlpinedotjs, SiFigma,
  SiPostgresql, SiMysql, SiMongodb,
  SiDocker, SiVercel, SiRailway, SiCloudflare, SiRender,
  SiGit, SiPostman, SiSwagger, SiSentry, SiVite
} from 'react-icons/si'
...
const skillCategories = [
  ...
  },
  {
    ...
    skills: [
      ...
      { name: 'shadcn/ui', icon: null, color: '' }, // No simple icon for shadcn
...
export function Skills() {
  ...
  return (
    <section id="skills" aria-label="Skills & Technologies" className="py-12 overflow-hidden">
      <div ref={ref} className={fadeClass}>
        <h2 className="text-2xl font-bold text-[var(--text-h)] mb-8">Skills & Technologies</h2>
          ...
          {skillCategories.map(({ label, icon, skills }) => (
            ...
            >
              <div className="flex items-center gap-3 mb-4">
                ...
                <h3 className="text-lg font-semibold text-[var(--text-h)]">{label}</h3>
              </div>
              <div className="flex flex-wrap gap-2">
                {skills.map((skill) => (
                  <span
                    key={skill.name}
                    className="flex items-center gap-2 px-3 py-1.5 text-xs font-medium rounded-lg bg-[var(--code-bg)] text-[var(--text)] border border-transparent hover:border-[var(--border)] transition-colors cursor-default"
                  >
                    {skill.icon && <skill.icon className={`w-3.5 h-3.5 ${skill.color}`} />}
                    {skill.name}
                  </span>
                ))}
              </div>
